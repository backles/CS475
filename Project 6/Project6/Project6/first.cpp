...
#define NUM_ELEMENTS		NMB*1024*100
...
int allMain(int);
void PrintCLError(cl_int);

int main( int argc, char *argv[] ){

	cout << endl << "MultArray Function" << endl;
	allMain(0);
	cout << endl << "ArrayMultADD Function" << endl;
	allMain(1);
	cout << endl << "Reduction" << endl;
	allMain(2);
	Sleep(5000);
}

int allMain(int option){
	...
	//size_t numberOfWorkGroups = NUM_ELEMENTS / LOCAL_SIZE;
	size_t dataSize = NUM_ELEMENTS * sizeof(float);
	//size_t cSize = numberOfWorkGroups * sizeof(float);
	...
	cl_mem dOption = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int), NULL, &status);
	if (status != CL_SUCCESS)
	...
	status = clEnqueueWriteBuffer(cmdQueue, dOption, CL_FALSE, 0, sizeof(int), &option, 0, NULL, NULL);
	if (status != CL_SUCCESS)
		fprintf(stderr, "clEnqueueWriteBuffer failed (2)\n");
	...
	cl_kernel kernel = clCreateKernel( program, "All", &status );
	if( status != CL_SUCCESS )
	...
	status = clSetKernelArg(kernel, 3, sizeof(cl_mem), &dOption);
	if (status != CL_SUCCESS)
	...
	cout << "The gigacalcs persecond are " << NUM_ELEMENTS/(time1-time0/1000000000) << ".";
	return 0;
}
