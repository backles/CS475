...
int main(int argc, char *argv[ ]){
	...
	int numProcessors, NUMT;
	double startTime, endTime;
	
	
	/*if(argc != 2){
		cout << "The number of parameters entered was not 2. Exiting!" << endl;
		return(1);
	}
	else{
		NUMT = atoi(argv[1]);
	...
	ofstream data("data.csv", ios::out | ios::app);
	for(int NUMT = 1; NUMT <= 10; NUMT++){//Number of threads
		for(int NUMBODIES = 100; NUMBODIES <=100; NUMBODIES += 100){//Number of bodies
			for(int NUMSTEPS = 100; NUMSTEPS <= 1000; NUMSTEPS += 100){//number of steps
				startTime = 0;
				endTime = 0;
				omp_set_num_threads(NUMT);
				numProcessors = omp_get_num_procs();
				//cout << "Have " << numProcessors << " processors." << endl;
				...
				double answer = (NUMBODIES*NUMBODIES*NUMSTEPS)/(endTime-startTime)/1000000;
				//cout << "MegaCalculations per second: " << answer << endl;
				//cout << "The time taken was: " << endTime - startTime << endl;
				cout << "Threads: " << NUMT << " NUMBODIES: " << NUMBODIES << " NUMSTEPS: " << NUMSTEPS << " Time: " << endTime-startTime << " Calcs: " << answer << endl;
				data << NUMT << "," << NUMBODIES << "," << NUMSTEPS << "," << endTime-startTime << "," << answer << endl;
			}
		}
	}
	data.close();
